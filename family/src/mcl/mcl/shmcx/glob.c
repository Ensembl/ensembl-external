...
,  zgglob_p o2
   ...
   ;  }
      ...
      ;  mxp = mclMatrixCopy(mx)
      ;  while (--i)
         {  mclMatrix* mxt = mclMatrixCompose(mxp, mx, 0)
         ;  mclMatrixFree(&mxp)
         ;  mxp = mxt
      ;  }
...
,  zgglob_p o2
   ...
   ;  }
      ...
      ;  if (x->N_cols != y->N_rows)
         {  zmTell
            (  'e'
            ,  "dimensions [%dx%d] [%dx%d] do not fit"
            ,  x->N_rows, x->N_cols
            ,  y->N_rows, y->N_cols
            )
         ;  return NULL
      ;  }
      ;  z = mclMatrixCompose(x, y, 0)
...
(  void
   ...
   ;  while ((kv = mcxHashWalkStep(hw)))
      ...
      ;  switch(glob->glype)
         ...
         :  zmTell
            (  'e'
            ,  "[" TOKEN_VARS "] " "(internal) unexpected object glype <%s>\n"
            ,  glob->glype
            )
         ;  what = "???"
      ;  }
...
(  zgglob_p glob
   ...
   ;  switch(glob->glype)
      ...
      ;  case glob__mx
      :  zmTell('v', "mtx %d x %d", glob->mx->N_rows, glob->mx->N_cols)
      ;  break

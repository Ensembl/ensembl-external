...
#
# BioPerl module for Bio::EnsEMBL::ExternalData::RikenSQL::DBAdaptor
#
...
=head2 get_RikenAdaptor

 Title   : get_RikenAdaptor
 Usage   :
 Function:
 Example :
 Returns : 
...
sub get_RikenAdaptor {
  my $self = shift;

  if( ! defined $self->{_RikenAdaptor} ) {
    $self->{_RikenAdaptor} = Bio::EnsEMBL::ExternalData::RikenSQL::RikenAdaptor->new( $self );
  }
  return $self->{_RikenAdaptor};
}
...
sub get_Ensembl_SeqFeatures_contig{
   ...
   return $self->get_RikenAdaptor->get_Ensembl_SeqFeatures_contig(@args);
...
sub get_Ensembl_SeqFeatures_clone{
   ...
   return $self->get_RikenAdaptor->get_Ensembl_SeqFeatures_clone(@args);
...
sub get_SeqFeature_by_id {
   ...
   return $self->get_RikenAdaptor->get_SeqFeature_by_id($id);

...
#
# BioPerl module for Bio::EnsEMBL::ExternalData::RikenSQL::RikenAdaptor
#
 ...
 #  print STDERR "$statement\n\n";
   ...
   while($sth->fetch) {
       ...
       if( $hid ne '__NONE__' ) {
		   ...
		   $out->end        ($end);
		   $out->strand     ($strand);
		   $out->source_tag ('riken');
		   $out->primary_tag('riken');
		   $out->id         ($fid);
		   $out->p_value    ($evalue)    if (defined $evalue);
		   $out->percent_id ($perc_id)   if (defined $perc_id); 
		   $out->phase      ($phase)     if (defined $phase);    
...
sub get_Ensembl_SeqFeatures_clone{
   my ($self) = @_;
	...
	#$self->throw("get_Ensembl_SeqFeatures_clone not implemented for riken database");
   return ();
}
...
sub get_Ensembl_Genes_clone {
	...
	# $self->throw("get_Ensembl_Genes_clone is not valid for the riken database");
...
sub get_SeqFeature_by_id {
   ...
   $self->throw("get_SeqFeature_by_id not implmented for the riken database");
...
sub db {
    ...
    if (defined($arg)) {
	if ($arg->isa("Bio::EnsEMBL::ExternalData::RikenSQL::DBAdaptor")) {
	    $self->{_db} = $arg;
	} else {
	    $self->throw("[$arg] is not a Bio::EnsEMBL::ExternalData::RikenSQL::DBAdaptor");
	}
    }
    return $self->{_db};
}
...
=head2 get_Ensembl_SeqFeatures_exon
 ...
 Usage   :
 Function:Gets all of the rikens features for a given exon
 Example :
